...
| `GET /api/appointments/by-client/{clientId}` | Povijest termina klijenta, paginirano, najnoviji prvi. Vraća `ClientAppointmentHistoryDto`, **ne** `AppointmentDto` — klijent-povijest pristup != roster pristup, vidi Model niže. |
...
| `POST /api/appointments/complete` | **"Upiši odrađeno"** — novi termin odmah u statusu `Completed`, naplata odmah po klijentu (`Settlements`, vidi Model niže). `409 APPOINTMENT_OVERLAP` za preklapanje. |
...
// 200 OK — puni AppointmentDto
{
  ...
  "bookings": [{
    "clientId": "…", "clientName": "Ivana Kovač",
    "amount": 20.00, "suggestedAmount": 20.00, "isAmountManuallyOverridden": false,
    "paymentMethod": null, "isPaid": false,
    "clientPackageId": null, "packageEntryDeducted": false, "packageEntryReturned": false
  }],
  "warnings": [],
  "createdAt": "…", "updatedAt": "…"
}
...
- Zajednička polja na `Appointment`: `StartsAt` (UTC, proizvoljno vrijeme), `DurationMinutes` (snapshot iz `Service.DefaultDurationMinutes`), `Status` (`Scheduled`/`Completed`/`Cancelled`/`NoShow`), `Note`. Appointment nosi ISKLJUČIVO okvir/resurs (usluga, vrijeme, trener, tvrtka, prostorija) — nema vlastito `Amount`/`PaymentMethod`/`IsPaid`.
- **Komercijalno stanje je Booking-razina, po klijentu** (od 2026-09-15): `Amount`/`SuggestedAmount`/`IsAmountManuallyOverridden` (predložena cijena iz centralnog `IPricingService`, ručna izmjena se bilježi po Bookingu), `PaymentMethod`, `IsPaid` žive na `Booking`, ne na `Appointment`. **Mješovito plaćanje na istom terminu je podržano** — npr. duo termin gdje Klijent A plaća paketom a Klijent B karticom. Individualni termin dopušta jednog ili više klijenata (npr. par/duo trening); duo/par varijante se rješavaju odabirom druge usluge iz kataloga (npr. "Individualni trening u paru"), ne posebnom logikom.
- **Naplata pri "Upiši odrađeno"/"complete" ide preko `Settlements`** (`AppointmentCompleteRequest.Settlements`, zamjenjuje stari zajednički `PaymentMethod` + `PackageSelections`) — lista `{ clientId, paymentMethod, amount?, clientPackageId?, isPaid? }`, TOČNO jedan unos po klijentu na terminu. `PaymentMethod=Package` zahtijeva `clientPackageId`, provjeren protiv `GET .../packages/eligible`. Paket uvijek forsira `IsPaid=true` bez obzira na `isPaid` (entitlement podmiruje obvezu); `Amount` i dalje nosi redovnu/predloženu cijenu i kod plaćanja paketom (retail vrijednost za izvještaje, ne 0).
- **Plaćanje iz paketa je po klijentu, ne po terminu**: kod duo termina svaki klijent skida ulazak iz svog vlastitog `ClientPackage`-a, neovisno o ostalima na istom terminu (`Booking.ClientPackageId`/`PackageEntryDeducted`).
...
- `AppointmentAuditLog` bilježi tko/kada za ručnu izmjenu iznosa, vraćanje ulaska iz paketa i promjenu trenera na terminu (isti obrazac kao `EmployeeAuditLog`) — kod grupnih termina isti zapis bilježi i automatsko vraćanje ulaska pri poništavanju prisutnosti (vidi modul Grupe).
- `ChangeType="EmployeeId"` bilježi svaku promjenu izvođača na pojedinom terminu (`PUT {id}` i `{id}/move`), s `OldValue`/`NewValue` kao GUID stringovima trenera (ili prazno ako termin prije nije imao dodijeljenog trenera) — npr. kad drugi trener uskoči na tuđu grupu bez diranja rasporeda/rostera, ovo je jedini trag da se to dogodilo.
- `GET /appointments/by-employee/{employeeId}` — povijest odrađenog po zaposleniku, najnoviji prvi, paginirano (isti obrazac kao `GetByClient`). Za razliku od `GetByClient` (koja namjerno uključuje i otkazane/no-show termine klijenta), ovdje se filtrira **samo `Status=Completed`** — otkazan/no-show termin nije nešto što je zaposlenik odradio. Nema provjere postojanja `employeeId` niti ownership-provjere (isto ponašanje kao `GetByClient`). Ne izračunava agregate (broj termina, sati) niti naplatu/isplatu zaposleniku — to je namjerno izvan opsega, ostavljeno za budući izvještajni sloj koji trenutno ne postoji. Vraća pun `AppointmentDto` (sa svim `Bookings`) jer je ovo pogled zaposlenika na SVOJE odrađene termine (roster pristup), ne klijent-povijest.
- **Klijent-povijest pristup != roster pristup**: `GetByClient` NE vraća `AppointmentDto.Bookings` (to bi otkrilo imena ostalih klijenata na istom grupnom/duo terminu). Umjesto toga vraća `ClientAppointmentHistoryDto` — Appointment-razina podaci (usluga, tvrtka, trener, `startsAt`, trajanje, status termina, `groupId`/`groupName`) plus SAMO Booking zahtjevanog klijenta, uklj. njegovu vlastitu naplatu (`Amount`/`PaymentMethod`/`IsPaid` — Booking-razina, ne dijeli se s ostalim klijentima na istom terminu), `bookingId`, `bookingStatus`, `clientPackageId`, `coverageType`, `packageEntryDeducted`/`Returned`, `bookingNote`, `bookingCancellationReason`. Puni roster s podacima svih klijenata na terminu ostaje dostupan isključivo preko `GET /appointments/{id}` i `GET /appointments/{appointmentId}/bookings` (isti `AppointmentsView` grant, ali admin/trener eksplicitno traži cijeli termin, ne povijest jednog klijenta).
...
- Grupni termin (`Form=Group`) nema vlastiti iznos na `Appointment` — svaki Booking dobiva svoju predloženu cijenu (`SuggestedAmount`) čim se generira/doda, `PaymentMethod`/`IsPaid` ostaju `null`/`false` do check-ina (vidi modul Grupe).
...
| `POST/PUT /api/groups`, `PATCH .../{id}/activate`\|`deactivate` | Definicija grupe (naziv, usluga, tvrtka, kapacitet, zadani trener, napomena). Kreiranje zahtijeva barem jedan slot. Deaktivacija zaustavlja buduće generiranje I otkazuje već generirane buduće termine (`Status=Scheduled`, `StartsAt` u budućnosti) — vidi Model niže. Aktivacija ih ne vraća natrag. |
...
- Deaktivacija grupe (`GroupService.SetActive`, prijelaz `IsActive: true → false`) unutar iste transakcije otkazuje sve njene termine sa `Status=Scheduled` i `StartsAt` u budućnosti (postavlja `Status=Cancelled`, `CancellationReason="Grupa deaktivirana"`) — svaki takav prijelaz se bilježi i u `AppointmentAuditLog` (`ChangeType="Status"`), isto kao ručno otkazivanje pojedinog termina. Već odrađeni (`Completed`) ili već otkazani/no-show termini se ne diraju. Ne vraćaju se automatski ulasci iz paketa (isto zadano ponašanje kao kod ručnog `Cancel` bez `ReturnEntryForClientIds`). Ponovna aktivacija grupe NE vraća te termine u `Scheduled` — treba ih ponovno generirati.
...
- Grupni termin nema vlastiti iznos na `Appointment` (`Status` ostaje `Scheduled` — nema "odrađeno" prijelaza na razini termina kao kod individualnog). Naplata je Booking-razina, po članu — vidi "Evidencija prisutnosti" niže. Pauze/praznici/otkazivanje pojedinog termina idu kroz postojeći mehanizam otkazivanja (`POST /api/appointments/{id}/cancel`), isto kao ponavljajući individualni.
- **Evidencija prisutnosti** (`Booking`, po terminu × klijentu): `Attended` (bool?, null = još nezabilježeno — mapira se na `BookingStatus`), `CoverageType` (`MonthlyPackage`/`SessionPackage`/`SinglePaid`) određuje kako je dolazak pokriven:
  - `MonthlyPackage` — klijent ima aktivan paket kod kojeg je relevantni brojač ulazaka `null` (neograničeno) → samo se bilježi prisutnost, ništa se ne skida. `PaymentMethod=Package`, `IsPaid=true`, `Amount`=predložena/retail cijena (entitlement podmiruje obvezu, ne znači da je iznos 0).
  - `SessionPackage` — klijent ima paket s konačnim brojem ulazaka → skida se jedan ulazak (`ClientPackageService.DeductEntry`, poštuje SharedPool/PerService iz Kataloga). Isto `PaymentMethod=Package`/`IsPaid=true` kao gore.
  - `SinglePaid` — gost/probni bez odgovarajućeg paketa → bilježi se prisutnost; naplata je EKSPLICITNA odluka osoblja (`SetGroupAttendanceRequest.PaymentMethod`/`Amount`/`IsPaid`) — izostanak znači "evidentirano, još neplaćeno" (isto ponašanje kao prije uvođenja naplate na grupne bookinge).
