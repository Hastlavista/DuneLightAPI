...
- `GET /appointments/by-employee/{employeeId}` — povijest odrađenog po zaposleniku, najnoviji prvi, paginirano (isti obrazac kao `GetByClient`). Za razliku od `GetByClient` (koja namjerno uključuje i otkazane/no-show termine klijenta), ovdje se filtrira **samo `Status=Completed`** — otkazan/no-show termin nije nešto što je zaposlenik odradio. Nema provjere postojanja `employeeId` niti ownership-provjere (isto ponašanje kao `GetByClient`). Ne izračunava agregate (broj termina, sati) niti naplatu/isplatu zaposleniku — to je namjerno izvan opsega, ostavljeno za budući izvještajni sloj koji trenutno ne postoji.
